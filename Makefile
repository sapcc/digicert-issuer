...
build-all: build/digicert-issuer

build/digicert-issuer: FORCE generate
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w $(GO_LDFLAGS)' -o build/digicert-issuer ./cmd/digicert-issuer
...
install: FORCE build/digicert-issuer
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/digicert-issuer "$(DESTDIR)$(PREFIX)/bin/digicert-issuer"
...
generate: install-controller-gen
	...
	@controller-gen crd rbac:roleName=digicert-issuer-role webhook paths="./..." output:crd:artifacts:config=config/crd/bases
...
goimports: FORCE install-goimports
	@printf "\e[1;36m>> goimports -w -local https://github.com/sapcc/digicert-issuer\e[0m\n"
	@goimports -w -local github.com/sapcc/digicert-issuer $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...))
...
help: FORCE
	...
	@printf "  \e[36mbuild/digicert-issuer\e[0m        Build digicert-issuer.\n"
